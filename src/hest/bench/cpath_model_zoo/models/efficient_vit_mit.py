...
from .attentional_poolers import AttentionalPooler
...
class EfficientViTKDStudentWithAdapter(nn.Module):
    def __init__(self, student, teacher_dim=1024, teacher_n_tokens=196):
        super().__init__()
        self.student = student
        if teacher_dim is None:
            teacher_dim = student.num_features
        if teacher_n_tokens is None:
            raise ValueError('teacher_n_tokens must be specified')
        
        self.projector = nn.Sequential(AttentionalPooler(context_dim=self.student.num_features,
                                         d_model=teacher_dim,
                                         n_queries=teacher_n_tokens + 1),
                                     nn.LayerNorm(teacher_dim))
    
    def forward(self, x):
        student_outtokens = self.student.forward_features(x) # torch.Size([B, 512, 7, 7])
        student_outtokens = rearrange(student_outtokens, 'b d h w -> b (h w) d')
        student_outtokens = self.projector(student_outtokens)
        # student_clstoken = student_outtokens[:, 0, :]
        # student_patchtokens = student_outtokens[:, 1:, :]
        return student_outtokens[:, 0, :]
...
def efficientvit_w_pooler(student_name = 'efficientvit_l2', student_kwargs = {}, 
                          projector_kwargs={'teacher_dim': 1024, 'teacher_n_tokens': 196}):
    if student_name == 'efficientvit_l2':
        student = efficientvit_l2(**student_kwargs)
    elif student_name == 'efficientvit_b2':
        student = efficientvit_b2(**student_kwargs)
    else:
        raise ValueError('invalid student name: {}'.format(student_name))
    if projector_kwargs is None:
        return student
    return EfficientViTKDStudentWithAdapter(student, **projector_kwargs)